...
##TODO: Add a custom fact
require "facter"

Facter.add(:primary_user) do
  setcode "cat /etc/primary_user"
end


Puppet::Type.type(:package).provide :appzip,
:parent => Puppet::Provider::Package do
  ...
  def self.instances_by_name
    Dir.entries("/var/db").find_all { |f|
      f =~ /^\.puppet_appzip_app_installed_/
    }.collect do |f|
      name = f.sub(/^\.puppet_appzip_app_installed_/, '')
  ...
  def install
    ...
    FileUtils.mkdir_p '/tmp/appzip_cache'
    ...
    chown "-R", "#{Facter[:primary_user].value}:staff", app_path
...
private
  ...
  def cached_path
    "/tmp/appzip_cache/#{@resource[:name]}.app.#{flavor}"
  end

  def receipt_path
    "/var/db/.puppet_appzip_app_installed_#{@resource[:name]}"
  end
